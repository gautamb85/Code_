...
def build_rnn(net_input=None, mask_input=None):
    ...
    l_last = lasagne.layers.SliceLayer(l_sum,-1,1) #this is for the phrase softmax
    
    Recc_emb =  lasagne.layers.get_output(l_sum, inputs={l_in: net_input, l_mask: mask_input})#.eval({X: x_dummy, Mask: mask})
    ...
    lstep = T.sum(mask_input, axis=1)#.eval({Mask : mask})
    lstep = T.cast(lstep, 'int32')#.eval()
    lstep = lstep.T

    l_attention = lasagne.layers.ReshapeLayer(l_attend, (n_batch, MAX_LENGTH))
    
    wts = lasagne.layers.get_output(l_attention, inputs={l_in: net_input, l_mask: mask_input})
    Wts = T.zeros([n_batch, MAX_LENGTH], dtype='float32')

    AWts = T.zeros([n_batch, MAX_LENGTH], dtype='float32')
    ...
    l_spk_softmax = lasagne.layers.DenseLayer(l_in2, num_units=NUM_SPEAKERS, nonlinearity=lasagne.nonlinearities.softmax)
